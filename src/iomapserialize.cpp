...
bool IOMapSerialize::saveHouseInfo()
{
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		...
		if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {}, {:s}", house->getId(), tfs::to_underlying(GUEST_LIST), db.escapeString(listText)))) {
				return false;
			}
		...
		if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
			if (!stmt.addRow(fmt::format("{:d}, {}, {:s}", house->getId(), tfs::to_underlying(SUBOWNER_LIST), db.escapeString(listText)))) {
				return false;
			}
