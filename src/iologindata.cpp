...
Account IOLoginData::loadAccount(uint32_t accno)
{
	...
	const std::string typeColumn = db.quoteIdentifier("type");
	DBResult_ptr result = db.storeQuery(fmt::format("SELECT id, name, password, {:s}, premium_ends_at FROM accounts WHERE id = {:d}", typeColumn, accno));
	if (!result) {
		return account;
	}
...
bool IOLoginData::loginserverAuthentication(const std::string& name, const std::string& password, Account& account)
{
	...
	DBResult_ptr result = db.storeQuery(fmt::format("SELECT id, name, password, secret, {:s}, premium_ends_at FROM accounts WHERE name = {:s}", typeColumn, db.escapeString(name)));
	if (!result) {
		return false;
	}
...
AccountType_t IOLoginData::getAccountType(uint32_t accountId)
{
	Database& db = Database::getInstance();
	const std::string typeColumn = db.quoteIdentifier("type");
	DBResult_ptr result = db.storeQuery(fmt::format("SELECT {:s} FROM accounts WHERE id = {:d}", typeColumn, accountId));
	if (!result) {
...
void IOLoginData::setAccountType(uint32_t accountId, AccountType_t accountType)
{
	Database& db = Database::getInstance();
	const std::string typeColumn = db.quoteIdentifier("type");
	db.executeQuery(fmt::format("UPDATE accounts SET {:s} = {:d} WHERE id = {:d}", typeColumn, static_cast<uint16_t>(accountType), accountId));
}
...
bool IOLoginData::preloadPlayer(Player* player, const std::string& name)
{
	...
	const std::string typeColumn = db.quoteIdentifier("type");
	DBResult_ptr result = db.storeQuery(fmt::format("SELECT p.id, p.account_id, p.group_id, a.{:s}, a.premium_ends_at FROM players as p JOIN accounts as a ON a.id = p.account_id WHERE p.name = {:s} AND p.deletion = 0", typeColumn, db.escapeString(name)));
	if (!result) {
		return false;
	}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT {:s}, {:s} FROM player_storage WHERE player_id = {:d}", keyColumn, valueColumn, player->getGUID())))) {
		do {
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	const std::string keyColumn = db.quoteIdentifier("key");
	const std::string valueColumn = db.quoteIdentifier("value");
	DBInsert storageQuery(fmt::format("INSERT INTO player_storage (player_id, {:s}, {:s}) VALUES ", keyColumn, valueColumn));
